...
def build_labeled_toc() -> pd.DataFrame:
    ...
    ab = ab[ab.match].drop(columns="match")
...
def build_adjacency_matrix() -> pd.DataFrame:
    ...
    return adj.T  # Orient datasets as records and variables as features


@st.experimental_memo
def cluster_dataset() -> pd.DataFrame:
    labeled_toc = build_labeled_toc()
    adj = build_adjacency_matrix()
    # Remove dataset unique features to speed-up clustering
    adj = adj.loc[:, adj.sum() > 1]
    ...
    return xy.join(labeled_toc).reset_index()
